...
def setup_argparse(parser):
    ...
    parser.add_argument('es_index_name', help='elasticsearch index name (mandatory, used to select es index 
file')
    ...
    parser.add_argument('@@search_timestamp', type=check_datetime, help='timestamp to use for search 
(optional, defaults to now)')
    parser.add_argument('@@search_timezone', type=check_timezone, default='-04:00', help='timezone to use 
for search (optional, defaults to +/-00:00)')
    parser.add_argument('@@es_index_date', type=check_date, help='elasticsearch index type (mandatory, but 
=null to not use it!  used to build index file name..dated - defaults to now date)')

    parser.add_argument('@@only_non_att_users', type=check_bool, help='show only cases where there are non 
att-users (default=false)')
...
def build_url_index_section(index_name, base_date, offset_array):
    """ for this use-case -> always work with two index files: 1) index of the day (i.e. gte_time) and 2) 
index of prior day """
    ...
    for i in range(len(result_array)):
        ...
        if args.only_non_att_users:
            if (not re.match(r'\w{2}\d{4}',the_who)) or (not re.match(r'\w{2}\d{4}',the_who2)):
                returned_string += ( '( ' + the_time + ' ; ' + the_who + ' ; ' + the_who2 + ' ; ' + 
the_reason + ' ) ' )
        else:
