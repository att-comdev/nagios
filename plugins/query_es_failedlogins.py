...
def setup_argparse(parser):
    ...
    parser.add_argument('range', type=check_range, help=range_help)

    parser.add_argument('company_uid_regex', help='Regular expression to recognize company uid')

    parser.add_argument('@@search_timestamp', type=check_datetime, help='timestamp to use for search (optional, defaults to now)')
    parser.add_argument('@@search_timezone', type=check_timezone, help='timezone to use for search (optional, defaults to +/-00:00)')
    ...
    parser.add_argument('@@only_non_company_users', type=check_bool, help='show only cases where there are non company-users (default=false)')
...
def build_detailed_string_from_results(args, result_array):
    ...
    for i in range(len(result_array)):
        ...
        if args.only_non_company_users:
            uid_regex = args.company_uid_regex
            if (not re.match(uid_regex,the_who)) or (not re.match(uid_regex,the_who2)):
...
def main():
    ...
    if args.debug:
        print('index name: ' + args.es_index_name)
        print('index type: ' + args.es_index_type)
        print('index date: ' + args.es_index_date.strftime("%Y-%m-%d"))
        print('lt_time: ' + lt_time.strftime("%Y-%m-%dT%H:%M:%S") )
        print('gte_time: ' + gte_time.strftime("%Y-%m-%dT%H:%M:%S") )
        print('search_timezone: ' + args.search_timezone)
        print('range: ' + str(args.range))
        print('critial_threshold: ' + str(args.critical_threshold))

        print('query url: ' + url)
        print('query data_json:')
        print(data_json)

    try:
        if args.usr and args.pwd:
            response = requests.post(url, data=json.dumps(data_json),
